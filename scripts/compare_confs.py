...
output_path = os.path.join(parent_dir, "outputdir", args.output_folder)
suffix = ''
print("Parsing results in: ", output_path)

with open(f'{output_path}/test_mols{suffix}.pkl', 'rb') as f:
    model_preds = pickle.load(f)

test_data = pd.read_csv(f'{parent_dir}/data/{dataset.upper()}/test_smiles_corrected.csv')  # this should include the corrected smiles
with open(f'{parent_dir}/data/{dataset.upper()}/test_mols.pkl', 'rb') as f:
...
np.save(f'{output_path}/stats{suffix}.npy', np.array([coverage_recall, amr_recall, coverage_precision, amr_precision, test_smiles], dtype=object))
...
output_text = (
    ...
    f'Recall AMR: Mean = {np.nanmean(amr_recall):.4f}, Median = {np.nanmedian(amr_recall):.4f}\n\n'
    f'Precision Coverage: Mean = {np.mean(coverage_precision_vals)*100:.2f}, Median = {np.median(coverage_precision_vals)*100:.2f}\n'
    f'Precision AMR: Mean = {np.nanmean(amr_precision):.4f}, Median = {np.nanmedian(amr_precision):.4f}\n'
)

print(output_text)

output_file_path = f'{output_path}/output.txt'
