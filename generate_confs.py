...
if model_parameters['hyperparams']['MolR_emb']:
    embed_network = model.load_pretrained_emb(model_parameters['hyperparams']['embed_path'])

if model_parameters['hyperparams']['utilize_deepergcn']:
    dataset = 'qm9_deepergcn'
    num_node_features = model_parameters['num_node_features']
    random_vec_dim = model_parameters['hyperparams']['random_vec_dim']
    embeddings_dim = model_parameters['hyperparams']['embeddings_dim']
    dg_molecular_property = model_parameters['hyperparams']['dg_molecular_property']

    deepergcn = DeeperGCN(input_dim=num_node_features+random_vec_dim, output_dim=1, edge_attr_dim=4+random_vec_dim, hidden_channels=embeddings_dim, num_layers=28).to(device)

    # Load deepergcn for the given molecular_property
    path = 'deepergcn/saved/' + dg_molecular_property + '_' + str(embeddings_dim) + '.pt'
...
for smi, n_confs in tqdm(test_data.values):
    ...
    model(data, inference=True, n_model_confs=n_confs*2, embed_network=embed_network, deepergcn=deepergcn)
...
# Create the full path to the subfolder inside outputdir
subfolder_path = os.path.join(outputdir, args.output_folder)
# Check if the subfolder exists, if not, create it
if not os.path.exists(subfolder_path):
    os.makedirs(subfolder_path)


file_path = os.path.join(subfolder_path, f'test_mols{suffix}.pkl')
