...
class GeoMol(nn.Module):
    def __init__(self, hyperparams, num_node_features, num_edge_features):
        ...
        self.no_h_mol = hyperparams['no_h_mol']
        ...
        self.embeddings_mlp = MLP(in_dim=hyperparams['embeddings_dim'], out_dim=self.model_dim, num_layers=hyperparams['h_mol_mlp']['n_layers']).to(self.device)
        self.combined_global = MLP(in_dim=self.model_dim+self.model_dim, out_dim=self.model_dim, num_layers=hyperparams['h_mol_mlp']['n_layers']).to(self.device)


        self.molr_x1 = MLP(in_dim=hyperparams['embeddings_dim'] + self.random_vec_dim, out_dim=self.model_dim, num_layers=hyperparams['gnn1']['n_layers']).to(self.device)
        self.molr_x2 = MLP(in_dim=hyperparams['embeddings_dim'] + self.random_vec_dim, out_dim=self.model_dim, num_layers=hyperparams['gnn2']['n_layers']).to(self.device)
        ...
        if self.random_alpha:
            self.alpha_mlp = MLP(in_dim=self.model_dim * num_concat + self.random_vec_dim, out_dim=1, num_layers=hyperparams['alpha_mlp']['n_layers']).to(self.device)
        else:
            self.alpha_mlp = MLP(in_dim=self.model_dim * num_concat, out_dim=1, num_layers=hyperparams['alpha_mlp']['n_layers']).to(self.device)
    ...
    def forward(self, data, ignore_neighbors=False, inference=False, n_model_confs=None, embed_network=None, deepergcn=None):
        ...
        if inference:
            ...
            try:
                ...
                fingerprint = None
            self.generate_model_prediction(data.x, data.edge_index, data.edge_attr, data.batch, data.chiral_tag, data.name, embed_network, fingerprint, deepergcn)
            return
        ...
        batched_model_stats = self.generate_model_prediction(x, edge_index, edge_attr, batch, chiral_tag, data.name, embed_network, data.fingerprint, deepergcn)
    ...
    def assign_neighborhoods(self, x, edge_index, edge_attr, batch, data):
        ...
        # move self.neighbors to CUDA
        for i in self.neighbors:
            self.neighbors[i] = self.neighbors[i].to(self.device)
        self.leaf_hydrogens = get_leaf_hydrogens(self.neighbors, x)
        self.dihedral_pairs = data.edge_index_dihedral_pairs.to(self.device)

        self.n_neighborhoods = len(self.neighbors)
        self.n_dihedral_pairs = len(self.dihedral_pairs.t())

        # mask for neighbors
        self.neighbor_mask = torch.zeros([self.n_neighborhoods, 4])
    ...
    def embed(self, x, edge_index, edge_attr, batch, smile_name, embed_network=None, fingerprint=None, deepergcn=None):
        x = x.to(self.device)
        edge_index = edge_index.to(self.device)
        ...
        # deepergcn input. Add stochasticity without repeating last dimension
        rand_x_unrepeated = rand_dist.sample([x.size(0), self.random_vec_dim]).squeeze(-1).to(self.device)
        rand_edge_unrepeated = rand_dist.sample([edge_attr.size(0), self.random_vec_dim]).squeeze(-1).to(self.device)
        x_unrepeated = torch.cat([x, rand_x_unrepeated], dim=-1)
        edge_attr_unrepeated = torch.cat([edge_attr, rand_edge_unrepeated], dim=-1)
        ...
        if self.hyperparams['MolR_emb'] and embed_network is None:
            raise Exception("embedding network is None but MolR_emb flag is True")

        if self.hyperparams['MolR_emb'] and embed_network:
            try:
                embeddings, flags, node_embeddings = embed_network.transform(smile_name)
                ...
                # print(f'embeddings.shape {embeddings.shape}')
                h_mol = embeddings.unsqueeze(1).repeat(1, self.n_model_confs, 1)
                # print(f'new embeddings.shape {h_mol.shape}')
                # print(f'old embeddings.shape {global_add_pool(x2, batch).shape}')

                # Process node embeddings
                rand_node = rand_dist.sample([node_embeddings.size(0), self.n_model_confs, self.random_vec_dim]).squeeze(-1).to(self.device)
                node_embed_rand = torch.cat([node_embeddings.unsqueeze(1).repeat(1, self.n_model_confs, 1), rand_node], dim=-1)

                if self.hyperparams['MolR_node_emb']:
                    x1 = self.molr_x1(node_embed_rand)
                    x2 = self.molr_x2(node_embed_rand)

                if self.hyperparams['combine_global']:
                    h_mol_original = self.h_mol_mlp(global_add_pool(x2, batch))
                    # print(f'h_mol.shape {h_mol.shape}')
                    # print(f'h_mol_original.shape {h_mol_original.shape}')
                    H_mol = torch.cat([h_mol, h_mol_original], dim=-1)
                    # print(f'H_mol.shape {H_mol.shape}')
                    h_mol = self.combined_global(H_mol)
                    # print(f'final_h_mol.shape {h_mol.shape}')

            except NotImplementedError:
                print("MolR failed inside embed during a forward call")
                # h_mol = global_add_pool(x2, batch)
                h_mol = self.h_mol_mlp(global_add_pool(x2, batch))

        elif self.hyperparams['utilize_fingerprints']:
            # Step 1: extract the global embeddings
            embeddings = fingerprint.reshape(fingerprint.shape[0]//1024, -1).float().to(self.device)
            # Step 2: transform the embeddings to GeoMol's shape
            embeddings = self.embeddings_mlp(embeddings)
            # Step 3: repeat the embeddings to match the number of conformers
            h_mol = embeddings.unsqueeze(1).repeat(1, self.n_model_confs, 1)
            if self.hyperparams['combine_global']:
                # Step 4: combine with GeoMol's global embeddings if combine_global is True
                h_mol_original = self.h_mol_mlp(global_add_pool(x2, batch))
                H_mol = torch.cat([h_mol, h_mol_original], dim=-1)
                h_mol = self.combined_global(H_mol)

        elif self.hyperparams['utilize_deepergcn']:
            # Step 1: extract the global embeddings
            _, embeddings, node_embeddings = deepergcn(x_unrepeated, edge_index, edge_attr_unrepeated, batch)
            # Step 2: transform the embeddings to GeoMol's shape
            embeddings = self.embeddings_mlp(embeddings)
            # Step 3: repeat the embeddings to match the number of conformers
            h_mol = embeddings.unsqueeze(1).repeat(1, self.n_model_confs, 1)
    ...
    def load_pretrained_emb(self, path, embedding='MolR'): # 'MolR/saved/sage_50'
        if embedding == 'MolR':
            network = MolEFeaturizer(path_to_model=path)
    ...
    def model_local_stats(self, x, chiral_tag):
        ...
        self.neighbor_mask = self.neighbor_mask.to(self.device)
    ...
    def align_dihedral_neighbors(self, dihedral_node_reps, dihedral_neighbors, batch, h_mol, dihedral_neighbor_reps):
        ...
        if self. random_alpha:
            ...
            if self.no_h_mol:
                alpha = self.alpha_mlp(torch.cat([dihedral_x_node_reps, dihedral_y_node_reps, rand_alpha], dim=-1)) + \
                    self.alpha_mlp(torch.cat([dihedral_y_node_reps, dihedral_x_node_reps, rand_alpha], dim=-1))
            else:
            ...
            if self.no_h_mol:
                alpha = self.alpha_mlp(torch.cat([dihedral_x_node_reps, dihedral_y_node_reps], dim=-1)) + \
                        self.alpha_mlp(torch.cat([dihedral_y_node_reps, dihedral_x_node_reps], dim=-1))
            else:
    ...
    def batch_model_stats(self, x1, x2, batch, h_mol, chiral_tag):
        ...
        chiral_tag = chiral_tag.to(self.device)
    ...
    def generate_model_prediction(self, x, edge_index, edge_attr, batch, chiral_tag, smile_name, embed_network=None, fingerprint=None, deepergcn=None):
        """
        ...
        # embed inputs
        x1, x2, h_mol = self.embed(x, edge_index, edge_attr, batch, smile_name, embed_network, fingerprint, deepergcn)
