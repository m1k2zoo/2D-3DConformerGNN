...
def train(model, loader, optimizer, device, scheduler, logger, epoch, writer, embed_network=None, deepergcn=None):
    failure_counts = 0
    model.train()
    ...
    for i, data in tqdm(enumerate(loader), total=len(loader)):
        ...
        if embed_network:
            with torch.no_grad():
                try:
                    embed_network.transform(data.name)
                except NotImplementedError:
                    print("MolR failed, ignoring this molecule.")
                    failure_counts+=1
                    pass

        optimizer.zero_grad()

        result = model(data, embed_network=embed_network, deepergcn=deepergcn) if epoch > 1 else model(data, ignore_neighbors=True, embed_network=embed_network, deepergcn=deepergcn)
        result.backward()
    ...
    print(f'number of molecules not embedded by MolR: {failure_counts}')
    return loss_all / len(loader)

@torch.no_grad()
def test_failures(model, loader, device, epoch, writer, embed_network=None, deepergcn=None):
    model.eval()
    loss_all = 0
    a, b, c, d, e = 0, 0, 0, 0, 0
    ...
    for i, data in tqdm(enumerate(loader), total=len(loader)):
        ...
        result = model(data, embed_network=embed_network, deepergcn=deepergcn)
        loss_all += result.item()
...
@torch.no_grad()
def test(model, loader, device, epoch, writer, embed_network=None, deepergcn=None):
    model.eval()
    loss_all = 0
    a, b, c, d, e = 0, 0, 0, 0, 0
