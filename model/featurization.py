...
class geom_confs(Dataset):
    def __init__(self, root, split_path, mode, transform=None, pre_transform=None, max_confs=10, use_fp=False):
        ...
        if self.use_fp:
            import pandas as pd
            self.smiles_to_fingerprint = pd.read_pickle(r'fingerprints/MAP4s_QM9.pickle')
        else:
            self.smiles_to_fingerprint = None
    ...
    def featurize_mol(self, mol_dic):
        ...
        if not confs[0]['rd_mol'].HasSubstructMatch(dihedral_pattern):
            return None
        # TODO: zeros(): argument 'size' (position 1) must be tuple of ints, not list
        # [self.max_confs, N, 3]) -> (self.max_confs, N, 3)
        ...
        # TODO: return the fingerprint
        if self.use_fp:
            fingerprint = self.smiles_to_fingerprint[name]
        else:
            fingerprint = None
        ...
        data = Data(x=x, z=z, pos=[pos], edge_index=edge_index, edge_attr=edge_attr, neighbors=neighbor_dict,
                    ...
                    degeneracy=conf['degeneracy'], mol=correct_mol, pos_mask=pos_mask, fingerprint=fingerprint)
        return data
...
class qm9_confs(geom_confs):
    def __init__(self, root, split_path, mode, transform=None, pre_transform=None, max_confs=10, use_fp=False):
        super(qm9_confs, self).__init__(root, split_path, mode, transform, pre_transform, max_confs, use_fp)
        self.types = {'H': 0, 'C': 1, 'N': 2, 'O': 3, 'F': 4}

class qm9_confs_deepergcn(geom_confs):
    def __init__(self, root, split_path, mode, transform=None, pre_transform=None, max_confs=10):
        super(qm9_confs_deepergcn, self).__init__(root, split_path, mode, transform, pre_transform, max_confs)
        # self.types = {'H': 0, 'C': 1, 'N': 2, 'O': 3, 'F': 4}
...
class drugs_confs(geom_confs):
    def __init__(self, root, split_path, mode, transform=None, pre_transform=None, max_confs=10, use_fp=False):
        super(drugs_confs, self).__init__(root, split_path, mode, transform, pre_transform, max_confs, use_fp)
        self.types = {'H': 0, 'Li': 1, 'B': 2, 'C': 3, 'N': 4, 'O': 5, 'F': 6, 'Na': 7, 'Mg': 8, 'Al': 9, 'Si': 10,
                      'P': 11, 'S': 12, 'Cl': 13, 'K': 14, 'Ca': 15, 'V': 16, 'Cr': 17, 'Mn': 18, 'Cu': 19, 'Zn': 20,
                      'Ga': 21, 'Ge': 22, 'As': 23, 'Se': 24, 'Br': 25, 'Ag': 26, 'In': 27, 'Sb': 28, 'I': 29, 'Gd': 30,
                      'Pt': 31, 'Au': 32, 'Hg': 33, 'Bi': 34}
...
def construct_loader(args, modes=('train', 'val')):
    ...
    loaders = []
    use_fingerprints = args.utilize_fingerprints
    for mode in modes:
        if args.dataset == 'qm9':
            dataset = qm9_confs(args.data_dir, args.split_path, mode, max_confs=args.n_true_confs, use_fp=use_fingerprints)
        elif args.dataset == 'qm9_deepergcn':
            dataset = qm9_confs_deepergcn(args.data_dir, args.split_path, mode, max_confs=args.n_true_confs)
...
drugs_types = {'H': 0, 'Li': 1, 'B': 2, 'C': 3, 'N': 4, 'O': 5, 'F': 6, 'Na': 7, 'Mg': 8, 'Al': 9, 'Si': 10,
               ...
               'Ga': 21, 'Ge': 22, 'As': 23, 'Se': 24, 'Br': 25, 'Ag': 26, 'In': 27, 'Sb': 28, 'I': 29, 'Gd': 30,
               'Pt': 31, 'Au': 32, 'Hg': 33, 'Bi': 34}
qm9_deepergcn_types = {'H': 0, 'Li': 1, 'B': 2, 'C': 3, 'N': 4, 'O': 5, 'F': 6, 'Na': 7, 'Mg': 8, 'Al': 9, 'Si': 10,
                'P': 11, 'S': 12, 'Cl': 13, 'K': 14, 'Ca': 15, 'V': 16, 'Cr': 17, 'Mn': 18, 'Cu': 19, 'Zn': 20,
                'Ga': 21, 'Ge': 22, 'As': 23, 'Se': 24, 'Br': 25, 'Ag': 26, 'In': 27, 'Sb': 28, 'I': 29, 'Gd': 30,
                'Pt': 31, 'Au': 32, 'Hg': 33, 'Bi': 34}
...
def featurize_mol_from_smiles(smiles, dataset='qm9'):
    ...
    if dataset == 'qm9':
        types = qm9_types
    elif dataset == 'qm9_deepergcn':
        types = qm9_deepergcn_types
    elif dataset == 'drugs':
        types = drugs_types
