...
def construct_conformers(data, model):
    ...
    for i, pair in enumerate(dihedral_pairs):
        ...
        if cycle_added:
            ...
            q_reorder = np.argsort([np.where(a == q_idx.cpu())[0][0] for a in torch.tensor(cycle_avg_indices)[q_coords_mask]])
        ...
        H_XY = rotation_matrix_inf_v2(p_coords, model.x_map_to_neighbor_y[i]).to(device)
        H_YX = rotation_matrix_inf_v2(q_coords, model.y_map_to_neighbor_x[i]).to(device)
...
def calculate_gamma(n_model_confs, dihedral_mask, c_ij, v_star, Sx, Sy, p_idx, q_idx, x_index, y_index,
    ...
    A_curr_tensor = torch.tensor([[1., -1., -1., 1.]]).to(device)
    A_curr_inv_ = A_curr.view(n_model_confs, 4)[:, [3, 1, 2, 0]] * A_curr_tensor
